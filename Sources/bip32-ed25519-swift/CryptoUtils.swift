...
public struct CryptoUtils {
    ...
    data.withUnsafeBytes {_ in 
        _ = hash.withUnsafeMutableBytes { hashBytes in
            CC_SHA512(hashBytes.baseAddress, CC_LONG(data.count), hashBytes.bindMemory(to: UInt8.self).baseAddress)
        }
    }
    return hash
}
    ...
    public static func hmacSha512(key: Data, data: Data) -> Data {
        ...
        key.withUnsafeBytes { keyBytes in
            data.withUnsafeBytes { dataBytes in
                _ = hmac.withUnsafeMutableBytes { hmacBytes in
    ...
    public static func sha256(data: Data) -> Data {
        ...
        data.withUnsafeBytes {_ in 
            _ = hash.withUnsafeMutableBytes { hashBytes in
                CC_SHA256(hashBytes.baseAddress, CC_LONG(data.count), hashBytes.bindMemory(to: UInt8.self).baseAddress)
            }
        }
        return hash
    }
